...
set( ax(1) ...
    ...
    , 'XScale', 'log' ...
    , 'XTick', [] ...
    , 'YTick', [15 20 25 30 40 50 60 80 100 120 130] ...
    );
...
for ix = [gearinches(:) ringix(:) cogix(:)]'
    ...
    for jx = neighbors'
        ...
        pct = (jx(1)/gi - 1) * 100;
        plot(ax(1), [rings(rix) rings(jx(2))], [gi jx(1)], '-', 'Color', colors(distance,:));
        h = text((rings(rix)+rings(jx(2)))/2, (gi+jx(1))/2, sprintf('%.2g%%', pct));
