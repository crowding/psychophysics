function this = autoobject(varargin)
    ...
    %
    %This is an object-oriented framework that I made up a few years before
    %the Mathworks introduced their own object oriented syntax. I
    %havent' had a reason to switch to 'sanctioned' matlab objects since
    %then. (they suffer from the same performance and memory problems.)
    %
    %Example:
    %
    ...
    %require the new boilerplate line:s
    ...
    function subs = subsrefize_(subs)
        if ischar(subs)
            ...
            try
                ...
                errorcause(lasterror, 'autoobject:invalidSubscript', 'Invalid subscript reference ".%s"', subs);
            end
        end
        ...
        try
            ...
            errorcause(lasterror, 'autoobject:invalidSubscript', 'Improper substruct');
        end
    end
