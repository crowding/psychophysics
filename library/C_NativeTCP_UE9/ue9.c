...
int openTCPConnection(char *ipAddress, int port)
{
  ...
  int window_size = 2048 * 1024;  //current window size is 128 kilobytes
  int rw = 0;
  int size = sizeof(rw);
  int err;

  err = getsockopt(socketFd, SOL_SOCKET, SO_SNDBUF, (char*)&rw, &size );
  fprintf(stderr, "send window size is %d\n", rw);

  err = getsockopt(socketFd, SOL_SOCKET, SO_RCVBUF, (char*)&rw, &size );
  fprintf(stderr, "receive window size is %d\n", rw);

  err = setsockopt(socketFd, SOL_SOCKET, SO_SNDBUF, (char *) &window_size, sizeof(window_size));
  err = getsockopt(socketFd, SOL_SOCKET, SO_SNDBUF, (char*)&rw, &size );
  fprintf(stderr, "set send window size of %d, got back %d\n", window_size, rw);

  err = setsockopt(socketFd, SOL_SOCKET, SO_RCVBUF, (char*)&window_size, sizeof(window_size));
  err = getsockopt(socketFd, SOL_SOCKET, SO_RCVBUF, (char*)&rw, &size );
  fprintf(stderr, "set recv window size of %d, got back %d\n", window_size, rw);
  #endif
...
long ehDIO_Feedback(int fd, uint8 channel, uint8 direction, uint8 *state)
{
  ...
  if(channel <=  7)
  {
    ...
    sendBuff[6] = tempByte;
    sendBuff[7] = ((tempDir) ? tempByte : 0);
    sendBuff[8] = ((tempState) ? tempByte : 0);
  }
  else if(channel <= 15)
  {
    tempByte = pow( 2, (channel - 8));
    sendBuff[9] = tempByte;
    sendBuff[10] = ((tempDir) ? tempByte : 0);
    sendBuff[11] = ((tempState) ? tempByte : 0);
  }
  else if(channel <= 19)
  {
    tempByte = pow( 2, (channel - 16));
    sendBuff[12] = tempByte;
    sendBuff[13] = ((tempDir) ? (tempByte*16) : 0);
    sendBuff[13] = ((tempState) ? tempByte : 0);
  }
  else if(channel <= 22)
  {
    tempByte = pow( 2, (channel - 20));
    sendBuff[14] = tempByte;
    sendBuff[15] = ((tempDir) ? (tempByte*16) : 0);
    sendBuff[15] = ((tempState) ? tempByte : 0);
  }
  else
  {
