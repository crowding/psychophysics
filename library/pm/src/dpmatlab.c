 ...
 *          A. Westphal (Jan 99, WIN32 stuff)
 *          S. Pawletta (Jan 99, rewritten)
 *          E. Svahn ( 
 *             Nov 00, -DPPARENT removed
 *                     -BLOCK,VM added. 
 *                     -Matlab 6 possible to add 
 *                     -Nice adjuster added - no longer relative to first Matlab proc 
 *             Jan 01  -Window title with hostname
 *             Mar 01  -Added support for redirection of standard output under Unix)
 */
...
#define OUTP_REDIR_PRE  "1>"
#define OUTP_REDIR_SUF  "2>&1" 
#endif
...
int main(int argc, char **argv) {
	...
	switch (argc) {
		...
		case 2:
			...
			printf("Usage: dpmatlab \n");
			printf("DISPLAY=...\n");
			printf("PMSPAWN_BLOCK=1 | 0\n");
			printf("PMSPAWN_WD=...\n");
			printf("PMSPAWN_RUNMODE=fg | bg\n");
			printf("PMSPAWN_TRY=...\n");
			printf("PMSPAWN_CATCH=...\n");
			printf("PMSPAWN_VM=vmid\n");
#define ARGC2ENV  7
			printf("4 | 5 | 6       (matlab version)\n");
			printf("normal | low | same  (priority)\n");
			printf("output filename (standard output directed to)\n");
#define ARGCTOTAL 11	/* one is added for argv[0] */
			exit(EXIT_SUCCESS);
...
#ifndef WIN32 /* redirect output if in background */
	if (!strcmp(getrunmode(),"bg")) {
	  append(CMD_ARGS_MAX, cmdargs, OUTP_REDIR_PRE);
	  append(CMD_ARGS_MAX, cmdargs, out_redir);
	  append(CMD_ARGS_MAX, cmdargs, OUTP_REDIR_SUF);
	}
#endif
	/*
	 * start matlab
...
adjustrunmode(char *cmd[], char *cmdargs[], char mode) {
	...
	if ( mode == 'c' ) {
		 ...
		 */
#ifndef NOUNSETENV
		  unsetenv("DISPLAY"); /* really delete environ var */
#else
		  putenv("DISPLAY=");		/* set to empty */
#endif
		  return;
		}
	}
...
adjustpriority(char *cmd[], char *cmdargs[], char *priority) {
  ...
  if ( !strcmp(priority,"same") ) {
    return; /*  Same prio as mother proc. */
  }
...
adjustmatlab(char *cmd[], char *cmdargs[], char *matlabver) {
	...
	/*	append(CMD_MAX,      cmd,     "/home/argon/ersva/pmmat_bg");
	append(CMD_ARGS_MAX, cmdargs, "/home/argon/ersva/pmmat_bg");
	append(CMD_ARGS_MAX, cmdargs, "");*/
	return;
}
