...
% patched by J. Ditterich, 2/02
% setting gui to 2 eliminates the final message box and prevents the dispatcher
% from cancelling active processes in case of a break from the GUI (which would
% block the system)
% 12/10/02 jd fixed problem showing up with MATLAB R13
...
pmeval(slaves,['pmclearbuf;pmsend(' int2str(pmid) ',0,''PMCACK'');'],+(~pmdconf.debug)); % fix for MATLAB R13
...
if pmdconf.gui
  ...
  for h = h0'
    if strcmp(get(h,'tag'),'dispatcher');
      if pmdconf.gui==1 % no final message box if gui is set to 2
...
if ~isempty(working_pmis)&pmdconf.gui~=2 % don't do it if gui is set to 2
