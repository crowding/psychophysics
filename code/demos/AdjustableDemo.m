...
...%        , 'radius', repmat([repmat(9, 1, crowderNumber) repmat(11, 1, crowderNumber)], 1,2) ...
    ...
    presets = {
          ...
          , { my_.n,                   5 ...  %two small locally opposed wheels
            ...
            { my_.n, 10 ...  %testing doing things by column?
            , my_.relativeContrast, 0 ...
            , my_.motion.dt, [0.1 0.1 0.1 0.1 1] ...
            , my_.motion.radius,       10 ...
            , my_.motion.dphase, .75 / 10 ...
            , my_.dphase_paused, 0 ...
            , my_.motion.x, 0 ...
    ...
    function distribute()
        ...
        if dphase_paused
            realdphase = dphase_temp;
        else
            realdphase = motion.getDphase();
        end
        
        if (relativeContrast == 1)
            motion.setVelocity(sign(realdphase .* this.property__(my_.motion.velocity(1))) .* motion.getVelocity());
        else
            motion.setVelocity(-sign(realdphase .* this.property__(my_.motion.velocity(1))) .* motion.getVelocity());
        end
...
%         end
        ...
        function zerodphase(h) %#ok
            if dphase_paused
                motion.setDphase(dphase_temp);
                dphase_paused = 0;
            else
                dphase_temp = motion.getDphase();
                motion.setDphase(0);
                dphase_paused = 1;
            end
        end
