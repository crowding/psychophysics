...
FEEDBACK_COMMAND_ = struct...
    ...
    ANALOG_IN_CHANNEL_ = struct ...
        ( 'enum_', uint8(0)...
        , 'Vref',       14 ...
        , 'GND',        15 ...
        ... %, 'Vref',       128 ... %not handled due to struct
        ... %, 'Vsupply',    132 ... %not handled due to struct
        , 'Temp',       133 ...
        ... % , 'GND',        136 ... %not handled due to struct
        ... %, 'Vsupply',    140 ... %not handled due to struct
        ... %, 'Temp',       141 ... %not handled due to struct
        );
    for i_ = setdiff(0:127, [14 128 132 133 15 136 140 141])
        ANALOG_IN_CHANNEL_.(sprintf('AIN%d', i_)) = i_;
    end
...
FEEDBACKALT_COMMAND_ = struct...
    ...
    , 'format', struct...
        ...
        , 'DAC1Enabled',        false...
        , 'AINMask',            false(1, 16)...
        , 'AIN14ChannelNumber', ANALOG_IN_CHANNEL_...
        , 'AIN15ChannelNumber', ANALOG_IN_CHANNEL_...
        , 'Resolution',         uint8(0)...
        , 'SettlingTime',       uint8(0)...
        , 'AINGain',            true(4, 16)...
    ...
    function r = feedbackAlt(varargin)
        ...
        %channel samplings.
        %
        %TODO: think about channel name translations here...
...
SINGLEIO_COMMAND_ = 4;
    ...
    function r = analogIn(channel, gain, resolution, settlingTime)
        ...
        r.value = ainCal_(r.AIN ./ 256, r.channel, enumLookup_(s.bipGain, ANALOG_IN_GAIN_), s.resolution);
    ...
    function value = ainCal_(ain, channel, gain, resolution)
        ...
        if ischar(channel)
            if isfield(ANALOG_IN_CHANNEL_, channel)
                channel = ANALOG_IN_CHANNEL_.(channel);
            else
                error('LabJackUE9:noSuchChannel', 'No such channel');
            end
        end 
    ...
    function value = aoutcal_(channel, volts)
        if ischar(channel)
            if isfield(ANALOG_OUT_CHANNEL_, out)
                channel = ANALOG_OUT_CHANNEL_.(channel);
            else
                error('LabJackUE9:noSuchChannel', 'No such channel');
            end
        end 
...
TIMERCOUNTER_COMMAND_ = struct...
    ...
    function r = timerCounter(varargin)
        ...
        % default to 0.
        % override UpdateReset by providing arguments named
        % 'UpdateReset.TimerX'.
        %
...
%% StreamConfig
    ...
    function r = streamConfig(varargin)
        ...
        %
        % 'Channels', an array of up to 128 channel numbers. Required.
        %   See below for special stream channel numbers.
        % 'Gains', an array of gains (must have an equal number of
        %   elements as Channels. See under analogIn for the values.
        %   Required.
        ...
        % 'DivideBy256', divides the stream clcok by 256.
