function varargout = arrayreduce(fn, varargin)
    %function [argsout{1:n}] = arrayreduce(fn, initialstate{1:n}, inputs{1:n+m}, ['UniformOutput', 1|0] )
    %
    %Is to REDUCE as ARRAYFUN is to MAP. Supports multiple input and output arguments;
    ...
    %
    %Note that error handlers won't work the way they do in ARRAYFUN.

    no = nargout;
    accum = varargin(1:no);

    arrayfun(@step, varargin{no+1:end});
